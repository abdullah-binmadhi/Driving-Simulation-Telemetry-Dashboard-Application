...
const HealthBar = ({ label, value, icon: Icon }: { label: string, value: number, icon: any }) => {
    ...
    if (normValue < 0.7) { color = 'bg-yellow-500'; textColor = 'text-yellow-400'; }
    if (normValue < 0.4) { color = 'bg-red-500'; textColor = 'text-red-400'; }

    return (
        <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-lg bg-slate-800 border border-slate-700 ${textColor}`}>
                <Icon size={16} />
            </div>
            <div className="flex-grow">
                <div className="flex justify-between mb-0.5">
                    <span className="text-[10px] font-medium text-slate-400 uppercase">{label}</span>
                    <span className={`text-xs font-bold ${textColor}`}>{(normValue * 100).toFixed(0)}%</span>
                </div>
...
const CarHealth: React.FC<CarHealthProps> = ({ damage }) => {
    ...
    return (
        <div className="bg-slate-900 rounded-2xl p-2 border border-slate-800 flex-grow">
            <h2 className="text-lg font-semibold mb-2 text-slate-300 flex items-center gap-2">
