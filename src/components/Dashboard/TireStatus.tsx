...
const Tire = ({ label, temp, wear }: { label: string, temp: number, wear: number }) => {
    ...
    if (wear < 0.5) wearColor = 'bg-yellow-500';
    if (wear < 0.2) wearColor = 'bg-red-500';
        ...
        <div className="flex flex-col items-center bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
            ...
            <div className="w-full h-1.5 bg-slate-700 rounded-full mt-2 overflow-hidden">
                <div
                    ...
                    style={{ width: `${Math.max(0, wear * 100)}%` }}
                />
            </div>
            <span className="text-[10px] text-slate-500 mt-1">{(wear * 100).toFixed(0)}%</span>
        </div>
    );
};
